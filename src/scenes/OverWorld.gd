...
func _process(delta):
	var w_offset: Vector2 = ( wind_direction + gravity_direction + (player.linear_velocity))
	var w_speed = w_offset.length()
	#var ang = w_offset.angle()
	#rain_sprite.rotation = -ang - PI*0.5
	uv_offset -= Vector3(w_offset.x, w_offset.y, 0) * delta * sprite_tr
	rain_script.set("shader_param/uv1_offset", uv_offset * Vector3(-1,1,1))
